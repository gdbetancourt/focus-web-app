...
const SCRAPPERS = [
  {
    ...
    description: "Extract medications from pharmaceutical company pipelines",
    icon: Pill,
    color: "#00ffaa"
  },
  {
...
export default function Prospect() {
  const [activeTab, setActiveTab] = useState("pharma_pipelines");
  ...
  // Running states
  const [runningScrappers, setRunningScrappers] = useState({});
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [statsRes, oppsRes, logsRes, runsRes, configsRes] = await Promise.all([
        api.get("/scrappers/stats"),
        api.get("/scrappers/opportunities?limit=200"),
        api.get("/scrappers/logs?limit=100"),
        api.get("/scrappers/logs/runs?limit=50"),
        api.get("/scrappers/config")
      ]);
      ...
      // Process configs into a map
      const configMap = {};
      (configsRes.data.configs || []).forEach(c => {
        configMap[c.scrapper_id] = c;
      });
      setConfigs(configMap);
      
      // Load schedules from localStorage or initialize
  ...
  const runScrapper = async (scrapperId) => {
    setRunningScrappers(prev => ({ ...prev, [scrapperId]: true }));
    try {
      ...
      const checkStatus = async () => {
        try {
          ...
          if (statusRes.data.status === "running") {
            ...
            setRunningScrappers(prev => ({ ...prev, [scrapperId]: false }));
            loadData();
  ...
  const saveSchedule = (scrapperId, schedule) => {
    const newSchedules = { ...schedules, [scrapperId]: schedule };
  ...
  const getOpportunitiesForScrapper = (scrapperId) => {
    return opportunities.filter(o => o.source_scrapper === scrapperId);
  };

  const getLogsForScrapper = (scrapperId) => {
    return logs.filter(l => l.scrapper_id === scrapperId);
  };

  const getRunsForScrapper = (scrapperId) => {
    return runs.filter(r => r.scrapper_id === scrapperId);
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64" data-testid="prospect-loading">
        <RefreshCw className="w-8 h-8 animate-spin text-[#ff3300]" />
      </div>
    );
  }

  const renderScrapperTab = (scrapper) => {
    const scrapperOpps = getOpportunitiesForScrapper(scrapper.id);
    const scrapperLogs = getLogsForScrapper(scrapper.id);
    const scrapperRuns = getRunsForScrapper(scrapper.id);
    const schedule = schedules[scrapper.id] || { enabled: false, days: [], hour: 9 };
    const config = configs[scrapper.id] || {};
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div 
              ...
              style={{ backgroundColor: `${scrapper.color}20` }}
            >
              <scrapper.icon className="w-6 h-6" style={{ color: scrapper.color }} />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-white">{scrapper.name}</h2>
              <p className="text-slate-400 text-sm">{scrapper.description}</p>
            </div>
          </div>
          <Button
            onClick={() => runScrapper(scrapper.id)}
            disabled={runningScrappers[scrapper.id]}
            className="btn-primary"
          >
            {runningScrappers[scrapper.id] ? (
        ...
        <div className="grid grid-cols-4 gap-4">
          <Card className="stat-card">
            <CardContent className="p-4">
              <p className="text-2xl font-bold text-white">{scrapperOpps.length}</p>
              <p className="text-xs text-slate-400">Total Results</p>
            </CardContent>
          </Card>
          <Card className="stat-card">
            <CardContent className="p-4">
              <p className="text-2xl font-bold" style={{ color: scrapper.color }}>
                {scrapperOpps.filter(o => o.status === 'new').length}
              </p>
          ...
          <Card className="stat-card">
            <CardContent className="p-4">
              <p className="text-2xl font-bold text-green-400">
                {scrapperRuns.filter(r => r.status === 'completed').length}
              </p>
          ...
          <Card className="stat-card">
            <CardContent className="p-4">
              <p className="text-2xl font-bold text-red-400">
                {scrapperRuns.filter(r => r.status === 'failed').length}
              </p>
          ...
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center justify-between text-white">
              <span className="flex items-center gap-2">
                <Calendar className="w-5 h-5" style={{ color: scrapper.color }} />
                Weekly Schedule
              </span>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setShowScheduleEdit(scrapper.id)}
                ...
                <div className="flex gap-1">
                  {DAYS.map(day => (
                    <div
                      ...
                      style={schedule.days?.includes(day.id) ? { backgroundColor: `${scrapper.color}30`, color: scrapper.color } : {}}
                    >
                      {day.short}
                    </div>
                  ))}
        ...
        <Card className="stat-card">
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center gap-2 text-white">
              <Activity className="w-5 h-5" style={{ color: scrapper.color }} />
              Recent Runs
            </CardTitle>
          </CardHeader>
          <CardContent>
            {scrapperRuns.length === 0 ? (
              <p className="text-slate-500 text-center py-4">No runs yet</p>
            ) : (
              <div className="space-y-2 max-h-[200px] overflow-y-auto">
                {scrapperRuns.slice(0, 10).map(run => (
        ...
        <Card className="stat-card">
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center justify-between text-white">
              <span className="flex items-center gap-2">
                <Users className="w-5 h-5" style={{ color: scrapper.color }} />
                Results ({scrapperOpps.length})
              </span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            {scrapperOpps.length === 0 ? (
              ...
              <div className="border border-[#222] rounded-lg overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {scrapperOpps.slice(0, 20).map(opp => (
                      <TableRow key={opp.id} className="border-[#222] hover:bg-[#111]">
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: `${scrapper.color}20` }}>
                              <Linkedin className="w-4 h-4" style={{ color: scrapper.color }} />
                            </div>
                            <div>
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-1">
                            <Button
                              ...
                              size="sm"
                              onClick={() => {
                                setEditSheetContact(opp);
                                setShowEditSheet(true);
                              }}
                              className="text-orange-400 hover:text-orange-300"
                              title="Editar contacto"
                              data-testid={`edit-contact-${opp.id}`}
                            >
        ...
        <Card className="stat-card">
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center gap-2 text-white">
              <FileText className="w-5 h-5" style={{ color: scrapper.color }} />
              Execution Logs
            </CardTitle>
          </CardHeader>
          <CardContent>
            {scrapperLogs.length === 0 ? (
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                {scrapperLogs.slice(0, 20).map(log => (
  ...
  return (
    <div className="space-y-6" data-testid="prospect-page">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-4xl font-black text-white tracking-tight">Prospect</h1>
          <p className="text-slate-400 mt-1">LinkedIn scrappers and lead generation tools</p>
        </div>
      ...
      {/* Tabs for each scrapper */}
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-4 bg-[#111] p-1 gap-1">
          {SCRAPPERS.map(scrapper => (
            <TabsTrigger
              key={scrapper.id}
              value={scrapper.id}
              className="flex items-center gap-2 data-[state=active]:bg-[#1a1a1a] data-[state=active]:text-white text-slate-500"
            >
              <scrapper.icon className="w-4 h-4" />
              <span className="hidden md:inline">{scrapper.name}</span>
            </TabsTrigger>
          ))}
        </TabsList>

        {SCRAPPERS.map(scrapper => (
          <TabsContent key={scrapper.id} value={scrapper.id} className="mt-6">
            {renderScrapperTab(scrapper)}
          </TabsContent>
        ))}
      </Tabs>

      {/* Schedule Edit Dialog */}
      <Dialog open={!!showScheduleEdit} onOpenChange={() => setShowScheduleEdit(null)}>
        <DialogContent className="bg-[#111] border-[#222]">
          ...
          {showScheduleEdit && (
            <div className="space-y-6 py-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  checked={schedules[showScheduleEdit]?.enabled || false}
                  onCheckedChange={(checked) => {
                    setSchedules(prev => ({
                      ...prev,
                      [showScheduleEdit]: { ...prev[showScheduleEdit], enabled: checked }
                    }));
                  }}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  {DAYS.map(day => {
                    const isSelected = schedules[showScheduleEdit]?.days?.includes(day.id);
                    return (
                      <button
                        key={day.id}
                        onClick={() => {
                          const currentDays = schedules[showScheduleEdit]?.days || [];
                          const newDays = isSelected
                            ? currentDays.filter(d => d !== day.id)
                            : [...currentDays, day.id];
                          setSchedules(prev => ({
                            ...prev,
                            [showScheduleEdit]: { ...prev[showScheduleEdit], days: newDays }
                          }));
                        }}
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={schedules[showScheduleEdit]?.hour?.toString() || "9"}
                  onValueChange={(v) => {
                    setSchedules(prev => ({
                      ...prev,
                      [showScheduleEdit]: { ...prev[showScheduleEdit], hour: parseInt(v) }
                    }));
                  }}
                >
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowScheduleEdit(null)} className="border-[#333] text-slate-300">
              Cancel
            </Button>
            <Button 
              onClick={() => saveSchedule(showScheduleEdit, schedules[showScheduleEdit])}
              className="btn-primary"
            >
              Save Schedule
            </Button>
      ...
      {editSheetContact && (
        <ContactSheet
          ...
          onSave={(updatedContact) => {
            setOpportunities(prev => prev.map(c => 
