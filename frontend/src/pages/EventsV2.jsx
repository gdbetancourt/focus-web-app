...
import ImportWizard from "./foundations/ImportWizard";
    ...
    <div className="space-y-6" data-testid="events-v2-page">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="p-3 rounded-lg bg-[#ff3300]/20">
            <CalendarDays className="w-6 h-6 text-[#ff3300]" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-white">Webinars & Events</h1>
            <p className="text-slate-500">Gestión de eventos con cronograma automático</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          {showupStats && showupStats.overall.total_events > 0 && (
            <div className="relative">
              ...
              {/* Dropdown History */}
              {showupHistoryOpen && (
                <div className="absolute right-0 top-full mt-2 w-80 bg-[#111] border border-[#333] rounded-lg shadow-xl z-50">
      ...
      {trafficStatus && (
        <Card className={`border ${
          trafficStatus.status === 'green' ? 'bg-green-500/5 border-green-500/30' :
          trafficStatus.status === 'yellow' ? 'bg-yellow-500/5 border-yellow-500/30' :
          trafficStatus.status === 'red' ? 'bg-red-500/5 border-red-500/30' :
          'bg-slate-500/5 border-slate-500/30'
        }`}>
          <CardContent className="py-3 px-4">
            <div className="flex items-center gap-3">
              <div className={`w-4 h-4 rounded-full ${getTrafficLightColor(trafficStatus.status)}`} />
              <span className="text-sm text-slate-300">{trafficStatus.message}</span>
              <Badge variant="outline" className="text-xs border-[#333]">
                {trafficStatus.total_events || 0} eventos
              </Badge>
            </div>
          </CardContent>
        </Card>
                ...
                <CardContent className="p-3">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className={`w-2 h-2 rounded-full flex-shrink-0 ${getTrafficLightColor(event.traffic_light?.status)}`} />
