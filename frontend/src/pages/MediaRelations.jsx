...
// Buyer Persona ID for Jerónimo (media contacts)
const JERONIMO_PERSONA_NAME = "Jerónimo";

// Opportunity states
const OPPORTUNITY_STATES = [
  { value: 'open', label: 'Oportunidad Abierta', color: 'bg-blue-500/20 text-blue-400', icon: Clock },
...
export default function MediaRelations() {
  ...
  // Load contacts with Jerónimo persona
  const loadMediaContacts = useCallback(async () => {
    try {
      setLoadingContacts(true);
      // Get all contacts and filter by buyer persona Jerónimo
      const res = await api.get('/contacts', {
        params: { buyer_persona: JERONIMO_PERSONA_NAME, limit: 500 }
      });
      ...
      try {
        ...
        const mediaContacts = allContacts.filter(c => 
          c.buyer_persona === JERONIMO_PERSONA_NAME || 
          c.buyer_persona?.toLowerCase() === 'jerónimo'
        );
        setContacts(mediaContacts);
      } catch (err) {
        toast.error('Error loading media contacts');
      }
    } finally {
  ...
  useEffect(() => {
    loadMediaContacts();
    loadOpportunities();
  }, [loadMediaContacts, loadOpportunities]);
  ...
  return (
    <div className="space-y-6" data-testid="media-relations-page">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">2.2.4 Media Relations</h1>
          <p className="text-slate-500">Media contacts (Buyer Persona: Jerónimo) and collaboration opportunities</p>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="contacts" className="mt-6">
          {/* Info Banner */}
          <Card className="stat-card border-blue-500/30 mb-6">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                <Users className="w-5 h-5 text-blue-400" />
                <div>
                  <p className="text-white text-sm">
                    These are contacts with Buyer Persona <strong className="text-blue-400">Jerónimo</strong> (media professionals).
                  </p>
                  <p className="text-slate-500 text-xs mt-1">
                    Add contacts via Prospecting (1.1.1.3) using Jerónimo keywords, or manually assign the persona.
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          {loadingContacts ? (
            <div className="text-center py-12">
              ...
              <p className="text-slate-500 mt-2">Loading media contacts...</p>
            </div>
          ) : filteredContacts.length === 0 ? (
            <Card className="stat-card">
              <CardContent className="p-8 text-center">
                <Users className="w-12 h-12 mx-auto mb-4 text-slate-600" />
                <h3 className="text-lg font-medium text-white mb-2">No Media Contacts Found</h3>
                <p className="text-slate-400 mb-4">
                  Add contacts with Buyer Persona "Jerónimo" via Prospecting or manual assignment.
                </p>
              </CardContent>
            </Card>
          ) : (
            <div className="grid gap-3">
              {filteredContacts.map(contact => (
                <Card key={contact.id} className="stat-card hover:border-blue-500/30 transition-colors">
                  <CardContent className="p-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <Badge className="bg-blue-500/20 text-blue-400 text-xs">
                              {contact.buyer_persona || 'Jerónimo'}
                            </Badge>
                          </div>
